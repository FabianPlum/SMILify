...
class SLEAPDataLoader:
                ...
                if "json" in metadata_group.attrs:
                    try:
                        ...
                        if "nodes" in metadata:
                            node_names = [node.get("name", f"node_{i}") for i, node in enumerate(metadata["nodes"])]
    ...
    def _extract_2d_keypoints_camera_dirs(
        self, camera_data: Dict[str, Any], frame_idx: int
        ...
        if "pred_points" in camera_data and "score" in instance_points.dtype.names:
            ...
            original_visible = (
                instance_points["visible"]
                if "visible" in instance_points.dtype.names
                else np.ones(len(instance_points), dtype=bool)
            )
    ...
    def _extract_2d_keypoints_session_dirs(
        self, camera_data: Dict[str, Any], frame_idx: int
    ) -> Tuple[np.ndarray, np.ndarray]:
                ...
                try:
                    with h5py.File(camera_h5_file, "r") as f:
                        if "video_path" in f:
                            ...
                            if candidate_path.exists():
                                print(
                                    f"Using video file from {camera_h5_file} for camera {camera_name}: {candidate_path}"
                                )
                                return self._read_video_frame(candidate_path, frame_idx)
    ...
    def plot_keypoints_2d(self, camera_name: str, frame_idx: int = 0, save_path: Optional[str] = None):
        ...
        if np.any(visible_mask):
            plt.scatter(
                keypoints_2d[visible_mask, 0],
                keypoints_2d[visible_mask, 1],
                c="red",
                s=50,
                alpha=0.7,
        ...
        if np.any(invisible_mask):
            plt.scatter(
                keypoints_2d[invisible_mask, 0],
                keypoints_2d[invisible_mask, 1],
                c="gray",
                s=30,
                alpha=0.5,
                label="Invisible keypoints",
            )
    ...
    def plot_keypoints_on_frame(self, camera_name: str, frame_idx: int = 0, save_path: Optional[str] = None):
        ...
        for i, (x, y) in enumerate(keypoints_2d):
            if i < len(keypoint_names):
                ...
                if visibility[i]:
                    ...
                    # Invisible keypoint
                    ax.scatter(x, y, c=[color], s=50, alpha=0.6, edgecolors="white", linewidth=1, zorder=5)
        ...
        ax.set_title(
            f"SLEAP 2D Keypoints with Skeleton - Camera: {camera_name}, Frame: {frame_idx}",
            fontsize=14,
            color="white",
        ...
        with h5py.File(slp_file, "r") as f:
            if "metadata" in f:
                ...
                if "json" in metadata_group.attrs:
                    ...
                    if top_nodes:
                        node_names = [node.get("name", f"node_{i}") for i, node in enumerate(top_nodes)]
            ...
            try:
                ...
                with h5py.File(analysis_file, "r") as f:
                    ...
                    if "edge_names" in f:
                        ...
                        edge_names = [(edge[0].decode("utf-8"), edge[1].decode("utf-8")) for edge in edge_names_data]
    ...
    def _create_joint_mapping(self):
        ...
        print(
            f"Creating joint mapping between {len(self.smal_joint_names)} SMAL joints and {len(sleap_keypoint_names)} SLEAP keypoints"
        )
    ...
    def map_keypoints_to_smal_model(
        self, keypoints_2d: np.ndarray, visibility: np.ndarray, image_size: Tuple[int, int]
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
...
def main():
    ...
    lookup_table_path = (
        "/home/fabi/DATA_LOCAL/2025-12-05-better-falkner-mice/TRAIN_SMIL_MICE/lookup_table_names_Falkner_MICE.csv"
    )
    shape_betas_path = (
        "/home/fabi/DATA_LOCAL/2025-12-05-better-falkner-mice/TRAIN_SMIL_MICE/lookup_table_PCs_Falkner_MICE.csv"
    )
    ...
    if Path(project_path).exists():
        ...
        if shape_betas_info["shape_betas_available"]:
            ...
            print(
                f"  Range: [{shape_betas_info['shape_betas_range'][0]:.3f}, {shape_betas_info['shape_betas_range'][1]:.3f}]"
            )
