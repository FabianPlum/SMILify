...
def _project_joints_to_views(
    ...
    proj = cameras.transform_points_screen(joints_flat.float(), image_size=screen_size)[:, :, [1, 0]]  # swap to (y, x)
...
class _TriangulatorStub:
    ...
    def __init__(self, image_size: int, device: torch.device):
        ...
        self._triangulate_joints_dlt = types.MethodType(MultiViewSMILImageRegressor._triangulate_joints_dlt, self)
...
class TestTriangulateJointsDLT:
    ...
    def test_perfect_cameras_recover_joints(self, sample_joints_3d, triangulator, device):
        ...
        fov_per_view, R_per_view, T_per_view, aspect_ratios = _make_cameras_around_origin(
            V, radius=3.0, fov_deg=60.0, device=device, batch_size=B
        )
        ...
        assert mean_error < 0.01, f"Mean triangulation error {mean_error:.6f} exceeds 0.01"

    def test_two_views_sufficient(self, sample_joints_3d, triangulator, device):
        ...
        assert error.max().item() < 0.1, f"Max error {error.max().item():.4f} with 2 views — should still be reasonable"

    def test_masked_views_respected(self, sample_joints_3d, triangulator, device):
        ...
        assert not valid_mask.any(), "With only 1 visible view, no joints should be valid"

    def test_partial_visibility(self, sample_joints_3d, triangulator, device):
    ...
    def test_view_mask(self, sample_joints_3d, triangulator, device):
        ...
        assert not valid_mask.any(), "Only 1 view enabled via view_mask — nothing should be valid"
...
class TestTriangulationGradients:
    ...
    def test_gradients_flow_to_cameras(self, sample_joints_3d, triangulator, device):
        ...
        triangulated, valid_mask = triangulator._triangulate_joints_dlt(
            kp_2d,
            torch.ones(B, V, n_joints, device=device),
            fov_per_view,
            R_per_view,
            T_per_view_grad,
            aspect_ratio_per_view=aspect_ratios,
    ...
    def test_no_gradient_to_target_joints(self, sample_joints_3d, triangulator, device):
...
class TestTriangulationLossComputation:
    ...
    def test_perfect_reconstruction_gives_near_zero_loss(self, sample_joints_3d, triangulator, device):
        ...
        assert tri_loss.item() < 1e-3, f"Loss with perfect cameras should be near zero, got {tri_loss.item():.6f}"

    def test_perturbed_cameras_increase_loss(self, sample_joints_3d, triangulator, device):
        ...
        loss_true = (diff_sq_true * valid_true.float().unsqueeze(-1)).sum() / (valid_true.sum().float() * 3 + 1e-8)
        ...
        loss_pert = (diff_sq_pert * both_valid.float().unsqueeze(-1)).sum() / (both_valid.sum().float() * 3 + 1e-8)

        assert loss_pert.item() > loss_true.item(), (
            f"Perturbed cameras should give higher loss: true={loss_true.item():.6f}, perturbed={loss_pert.item():.6f}"
        )

    def test_gradient_descent_reduces_loss(self, sample_joints_3d, triangulator, device):
        ...
        T_optim = [(t + 0.2 * torch.randn_like(t)).requires_grad_(True) for t in T_per_view]
        ...
        for _ in range(20):
            ...
            loss = (diff_sq * valid.float().unsqueeze(-1)).sum() / (valid.sum().float() * 3 + 1e-8)
            loss.backward()
            optimizer.step()
            losses.append(loss.item())

        assert losses[-1] < losses[0] * 0.5, (
            f"20 steps of Adam should reduce loss by at least 50%: initial={losses[0]:.6f}, final={losses[-1]:.6f}"
        )
...
class TestTriangulationEdgeCases:
    ...
    def test_no_nan_with_zero_visibility(self, triangulator, device):
        ...
        assert torch.isfinite(triangulated).all(), (
            "Triangulated values should be finite even for invisible joints (damped solve should prevent NaN)"
        )
