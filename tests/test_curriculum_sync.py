...
CONFIG_PATH = os.path.join(
    os.path.dirname(__file__), "..", "smal_fitter", "neuralSMIL", "configs", "examples", "multiview_sticks.json"
)
...
def test_curriculum_loading():
    ...
    new_config = load_config(config_file=CONFIG_PATH, cli_overrides={}, expected_mode="multiview")
    ...
    TrainingConfig.LOSS_CURRICULUM = {
        ...
        "curriculum_stages": [
            (epoch, dict(updates)) for epoch, updates in sorted(new_config.loss_curriculum.curriculum_stages.items())
        ],
    }
