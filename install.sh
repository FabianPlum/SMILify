...
# 1b. tmux — handy for detaching long-running run.ai training/inference jobs.
...
install_tmux() {
    ...
    if command -v apt-get >/dev/null 2>&1 \
        && $SUDO apt-get update -qq && $SUDO apt-get install -y tmux; then
        echo "==> tmux installed ($(tmux -V))"
        return 0
    fi
    echo "==> apt-get unavailable/failed — falling back to conda-forge" >&2
    if conda install -n "$ENV_NAME" -y -c conda-forge tmux; then
        echo "==> tmux installed via conda-forge ($(tmux -V 2>/dev/null))"
        return 0
    fi
    echo "==> WARNING: could not install tmux automatically; install it manually if needed." >&2
    return 0
}
install_tmux
