...
def create_interactive_plot(df, title, output_dir=None):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=df["label"],
            hovertemplate=("<b>%{text}</b><br>" + "PC1: %{x:.3f}<br>" + "PC2: %{y:.3f}<br>" + "<extra></extra>"),
    ...
    for species in unique_species:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=species_data["PC2"],
                mode="markers",
                marker=dict(size=12, color=color_map[species], line=dict(width=1, color="white")),
                name=species,
...
Examples:
  ...
  python plot_pca_data.py data.csv --enhanced
    ...
    parser.add_argument("filepath", help="Path to the CSV file containing PCA XY data")

    parser.add_argument("--output-dir", "-o", help="Directory to save the plots (default: same as input file)")

    parser.add_argument(
        "--enhanced", "-e", action="store_true", help="Create enhanced plot with species-based coloring"
    )
