...
def load_stanford_sequence(STANFORD_EXTRA, image_name, crop_size):
    ...
    def get_seg_from_entry(entry):
        ...
        rle = {"size": [entry["img_height"], entry["img_width"]], "counts": entry["seg"]}
    ...
    raw_joints = np.concatenate([np.array(loaded_data["joints"]), [[0.0, 0.0, 0.0]]], axis=0)

    sil_img, rgb_img, landmarks = crop_to_silhouette(
        loaded_data["seg_data"], loaded_data["img_data"] / 255.0, raw_joints[:, [1, 0]], crop_size
    )
...
def load_SMIL_sequence(SMIL_COCO, image_name, crop_size, alt_seg=True, use_crop=False):
    """
    ...
    if config.DEBUG:
        ...
        for k, (key_point, name) in enumerate(zip(new_joint_locs, config.joint_names)):
            ...
            if k in config.TORSO_JOINTS:
                img_copy = cv2.circle(
                    img_copy, (key_point[1], key_point[0]), radius=3, color=(255, 0, 255), thickness=-1
                )
    ...
    if use_crop:
        sil_img, rgb_img, landmarks = crop_to_silhouette(seg_data, img_data, new_joint_locs, crop_size)
    else:
