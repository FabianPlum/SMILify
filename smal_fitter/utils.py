...
def crop_to_silhouette(sil_img, rgb_img, joints, target_size):
    ...
    y_min, y_max, x_min, x_max = (
        np.amin(foreground_pixels[0]),
        np.amax(foreground_pixels[0]),
        np.amin(foreground_pixels[1]),
        np.amax(foreground_pixels[1]),
    )
    ...
    square_sil = pad_sil[
        centre_y - square_half_side_length : centre_y + square_half_side_length,
        centre_x - square_half_side_length : centre_x + square_half_side_length,
    ]
    square_rgb = pad_rgb[
        centre_y - square_half_side_length : centre_y + square_half_side_length,
        centre_x - square_half_side_length : centre_x + square_half_side_length,
...
def perspective_proj_withz(X, cam, offset_z=0, cuda_device=0, norm_f=1.0, norm_z=0.0, norm_f0=0.0):
    """
    X: B x N x 3
    cam: B x 3: [f, cx, cy]
