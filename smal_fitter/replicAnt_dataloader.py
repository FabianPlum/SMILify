    ...
    try:
        ...
        if "View Matrix" in camera_data:
            ...
            camera_view_matrix = np.array(
                [
                    [
                        view_matrix["xPlane"]["x"],
                        view_matrix["yPlane"]["x"],
                        view_matrix["zPlane"]["x"],
                        view_matrix["wPlane"]["x"],
                    ],
                    [
                        view_matrix["xPlane"]["y"],
                        view_matrix["yPlane"]["y"],
                        view_matrix["zPlane"]["y"],
                        view_matrix["wPlane"]["y"],
                    ],
                    [
                        view_matrix["xPlane"]["z"],
                        view_matrix["yPlane"]["z"],
                        view_matrix["zPlane"]["z"],
                        view_matrix["wPlane"]["z"],
                    ],
                    [
                        view_matrix["xPlane"]["w"],
                        view_matrix["yPlane"]["w"],
                        view_matrix["zPlane"]["w"],
                        view_matrix["wPlane"]["w"],
                    ],
                ]
            )
        else:
...
def visualize_3d_scene(
    camera_translation, camera_intrinsics, subject_3d_locations, global_subject_rotation, camera_view_matrix
):
    """
    ...
    plot_coordinate_system(ax, [0, 0, 0], root_rotation, scale=5)

    # Plot camera
    plot_camera(
        ax, camera_translation, camera_intrinsics["FOV"], camera_view_matrix, root_rel=root_location_transcribed
    )
...
def plot_coordinate_system(ax, origin, rotation, scale=1.0):
    ...
    for i, (color, label) in enumerate(zip(["r", "g", "b"], ["X", "Y", "Z"])):
        ...
        ax.quiver(
            ...
            origin[1],
            origin[2],
            rotated_axis[0] * scale,
            rotated_axis[1] * scale,
            rotated_axis[2] * scale,
            color=color,
            label=f"{label} axis",
        )
...
def convert_camera_parameters(
    camera_translation, camera_view_matrix, subject_location=None, scale_factor=1.0, camera_rotation=None
):
    """
...
if __name__ == "__main__":
    ...
    json_file_path = os.path.abspath(
        os.path.join(current_dir, "..", "data", "replicAnt_trials", "TEST_ANGLES", "TEST_ANGLES_00.json")
    )
    ...
    if not os.path.exists(json_file_path):
        ...
        try:
            ...
            (
                ...
                subject_rotations,
                camera_view_matrix,
            ) = get_iteration_data(json_file_path)
    ...
    for key in subject_rotations:
        ...
        rodrigues_angle = unreal_euler_to_pytorch3d_rodrigues(
            pitch=subject_rotations[key]["y"], yaw=subject_rotations[key]["z"], roll=subject_rotations[key]["x"]
        )
    ...
    global_rotation = unreal_euler_to_pytorch3d_rodrigues(
        pitch=subject_rotations["b_t"]["y"], yaw=subject_rotations["b_t"]["z"], roll=-subject_rotations["b_t"]["x"]
    ...
    camera_location, camera_rotation = convert_camera_parameters(
        camera_translation,
        camera_view_matrix,
