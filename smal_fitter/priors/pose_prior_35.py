...
class Prior(object):
    def __init__(self, prior_path, device):
        ...
        if len(ignore_names) > 0:
            ...
            ignore_inds = np.hstack([np.array(ig * 3) + np.arange(0, 3) for ig in ignore_joints])
...
if __name__ == "__main__":
    ...
    seq_path_silvia = join(
        data_dir, "lioness_more_torso_parts_new_feet_new_back_legs_animation_mirrored_poses_prior.pkl"
    )  #'lioness_jaw_tail_straight_eyes_filled_animation_mirrored_poses.pkl')
    ...
    if FIX_TAIL:
        ...
        tail_inds = np.hstack([np.array(ig * 3) + np.arange(0, 3) for ig in tail_joints])
    ...
    if not exists(out_path):
        ...
        if "body_indept_limbstips" in out_path:
            ...
            for ind in indept_joints:
                istart = 3 + 3 * ind
                iend = 3 + 3 * ind + 3
                pose_cov[:istart, istart:iend] = 0.0
                pose_cov[iend:, istart:iend] = 0.0
                pose_cov[istart:iend, :istart] = 0.0
                pose_cov[istart:iend, iend:] = 0.0
            ...
            pose_invcov = np.linalg.inv(
                np.cov(pose_samples[:, use_ind].T) + 0.00001 * np.eye(pose_samples[:, use_ind].shape[1])
            )
            pic = np.linalg.cholesky(pose_invcov)  # Remove Ch wrapper
