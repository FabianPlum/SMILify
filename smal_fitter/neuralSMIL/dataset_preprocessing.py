...
class DatasetPreprocessor:
    ...
    def __init__(
        ...
        self.ignored_joints_config = ignored_joints_config or {
            "ignored_joint_names": [],
            "verbose_ignored_joints": False,
        }
            ...
            for joint_name in ignored_joint_names:
                if joint_name in self.joint_name_to_index:
                    ...
                    if self.ignored_joints_config.get("verbose_ignored_joints", False):
                        print(
                            f"Will ignore joint '{joint_name}' (index {self.joint_name_to_index[joint_name]}) during preprocessing"
                        )
                else:
    ...
    def process_single_sample(self, json_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            visibility = self._compute_keypoint_visibility(y_data["keypoints_2d"], x_data["input_image_mask"])
    ...
    def process_dataset(
        self, input_dir: str, output_path: str, num_workers: int = 4, batch_size: int = 32
    ) -> Dict[str, Any]:
        """
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            # Submit all tasks
            future_to_path = {
                executor.submit(self.process_single_sample, json_path): json_path for json_path in json_files
            }
    ...
    def _save_to_hdf5(self, samples: List[Dict[str, Any]], output_path: str):
        ...
        with h5py.File(output_path, "w") as f:
            ...
            required_params = ["global_rot", "joint_rot", "betas", "trans", "fov", "cam_rot", "cam_trans"]
            ...
            aux_group.create_dataset(
                "original_paths", shape=(num_samples,), dtype=h5py.special_dtype(vlen=str), chunks=chunk_shape
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--silhouette_threshold", type=float, default=0.1, help="Minimum silhouette coverage (default: 0.1)"
    )
    parser.add_argument("--target_resolution", type=int, default=224, help="Target image resolution (default: 224)")
    parser.add_argument(
        "--backbone_name", default="vit_large_patch16_224", help="Backbone name for resolution selection"
    )
    parser.add_argument(
        "--rotation_representation", choices=["6d", "axis_angle"], default="6d", help="Rotation representation"
    )
    parser.add_argument("--min_visible_keypoints", type=int, default=5, help="Minimum visible keypoints required")
    parser.add_argument("--chunk_size", type=int, default=8, help="HDF5 chunk size (batch size)")
    ...
    # Process dataset
    try:
        stats = preprocessor.process_dataset(args.input_dir, args.output_path, num_workers=args.num_workers)
