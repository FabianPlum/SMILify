...
def test_rotation_conversion_roundtrip(y_data: Dict[str, Any], device: str = "cpu", tolerance: float = 1e-4):
    ...
    return {"success": all_passed, "components": results, "tolerance": tolerance}


def visualize_keypoints_comparison(
    model, predicted_params, y_data, x_data, backbone_name, dataset, output_dir="visualizations"
):
    """
    ...
    rendered_joints, _ = model._compute_rendered_outputs(
        predicted_params, compute_joints=True, compute_silhouette=False
    )
    ...
    print(
        f"{'Joint':<5} {'GT Y':<8} {'GT X':<8} {'Rend Y':<8} {'Rend X':<8} {'Diff Y':<8} {'Diff X':<8} {'Magnitude':<10}"
    )
    print("-" * 75)
    for i in range(min(5, len(target_keypoints))):
        if target_visibility[i] > 0:
            ...
            print(
                f"{i:<5} {gt_y:<8.4f} {gt_x:<8.4f} {rend_y:<8.4f} {rend_x:<8.4f} {diff_y:<8.4f} {diff_x:<8.4f} {magnitude:<10.4f}"
            )
...
def test_3d_keypoint_alignment(model, predicted_params, y_data, device, tolerance=0.1):
    ...
    print(
        f"{'Joint':<5} {'GT X':<8} {'GT Y':<8} {'GT Z':<8} {'Model X':<8} {'Model Y':<8} {'Model Z':<8} {'Distance':<10} {'Status':<8}"
    )
    print("-" * 95)
    ...
    for i in range(num_joints):
        ...
        if i < 10:
            print(
                f"{i:<5} {gt_joint[0]:<8.3f} {gt_joint[1]:<8.3f} {gt_joint[2]:<8.3f} "
...
def visualize_3d_keypoint_alignment(
    gt_keypoints_3d, model_joints_3d, joint_errors, output_dir="test_visualizations", tolerance=0.1
):
    """
    ...
    ax1.scatter(
        gt_valid[:, 0], gt_valid[:, 1], gt_valid[:, 2], c="blue", s=50, alpha=0.7, label="Ground Truth", marker="o"
    )
...
def visualize_silhouette_comparison(model, predicted_params, x_data, output_dir="visualizations"):
    ...
    _, rendered_silhouette = model._compute_rendered_outputs(
        predicted_params, compute_joints=False, compute_silhouette=True
    )
    ...
    left_image[:, :, 1] = np.maximum(
        left_image[:, :, 1], target_silhouette if len(target_silhouette.shape) == 2 else target_silhouette[:, :, 0]
    )
...
def test_ground_truth_loss(
    ...
    if data_path is None:
        possible_paths = ["data/replicAnt_trials/replicAnt-x-SMIL-OmniAnt-Simple100k-noScale"]
    ...
    try:
        ...
        dataset = replicAntSMILDataset(
            data_path, rotation_representation=rotation_representation, backbone_name=backbone_name
        )
        if len(dataset) == 0:
        ...
        param_losses = {k: v.item() for k, v in loss_components.items() if k not in ["keypoint_2d", "silhouette"]}
        ...
        if enable_visualization:
            ...
            if "keypoints_2d" in y_data and "keypoint_visibility" in y_data:
                visualize_keypoints_comparison(
                    model, predicted_params, y_data, x_data, backbone_name, dataset, output_dir
                )
            ...
            if test_3d_keypoints and "3d_keypoints" in test_results:
                try:
                    ...
                    if (
                        "gt_keypoints_3d" in keypoint_3d_result
                        and "model_joints_3d" in keypoint_3d_result
                        and "joint_errors" in keypoint_3d_result
                    ):
...
def main():
    ...
    parser.add_argument(
        "--data_path", type=str, default=None, help="Path to the dataset (optional, will try default paths)"
    )
    parser.add_argument("--sample_idx", type=int, default=0, help="Index of the sample to test")
    parser.add_argument(
        "--tolerance_keypoints", type=float, default=1e-2, help="Maximum acceptable loss value for keypoints"
    )
    parser.add_argument(
        "--tolerance_silhouette", type=float, default=0.05, help="Maximum acceptable loss value for silhouette"
    )
    parser.add_argument(
        "--tolerance_3d_keypoints",
        type=float,
        default=1,
        help="Maximum acceptable distance for 3D keypoint alignment (model units)",
    )
    parser.add_argument("--device", type=str, default="cpu", help="Device to run on (cpu or cuda)")
    parser.add_argument("--no-visualization", action="store_true", help="Disable visual comparisons generation")
    parser.add_argument("--no-ue-scaling", action="store_true", help="Disable UE scaling (override dataset default)")
    parser.add_argument(
    ...
    parser.add_argument("--no-rotation-test", action="store_true", help="Disable rotation conversion round-trip test")
    parser.add_argument("--no-3d-keypoint-test", action="store_true", help="Disable 3D keypoint alignment test")
    parser.add_argument(
        "--backbone",
        type=str,
        default="resnet152",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Shape family index (overrides config.SHAPE_FAMILY). Only needed if --smal-file is not provided.",
    )
