...
class MemoryOptimizer:
    ...
    def optimize_model_for_memory(
        ...
        if current_memory_gb > self.target_memory_gb:
            recommendations["memory_optimizations"].append(
                f"Current memory usage ({current_memory_gb:.1f}GB) exceeds target ({self.target_memory_gb}GB)"
            )
    ...
    def get_optimal_batch_size(self, model: nn.Module, input_shape: tuple, max_batch_size: int = 32) -> int:
        """
