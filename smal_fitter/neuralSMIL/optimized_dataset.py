...
class OptimizedSMILDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if "rotation_representation" in self.metadata and self.rotation_representation != self.original_rotation_repr:
            print(
                f"Warning: Requested rotation representation '{self.rotation_representation}' "
        ...
        print(
            f"Loaded optimized dataset: {self.num_samples} samples, "
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        x_data = {
            ...
            "input_image_mask": silhouette_mask.squeeze(0)
            if silhouette_mask.ndim == 3
            else silhouette_mask,  # Remove channel dimension if present
        }
    ...
    def validate_sample(self, idx: int, verbose: bool = True) -> Dict[str, bool]:
        ...
        try:
            ...
            results = {
                ...
                "image_shape_correct": x_data["input_image_data"].shape
                == (self.dataset.target_resolution, self.dataset.target_resolution, 3),
                "image_range_correct": 0.0 <= x_data["input_image_data"].min()
                and x_data["input_image_data"].max() <= 1.0,
