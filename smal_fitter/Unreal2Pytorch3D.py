...
def plot_3D_points(pose_data, input_image):
    ...
    display_points_3D = [[pose_data[key]["3DPos"]["x"],
                          pose_data[key]["3DPos"]["y"],
                          pose_data[key]["3DPos"]["z"]] for key in pose_data.keys()]
...
if __name__ == '__main__':
    ...
    cx, cy, fx, fy = parse_camera_intrinsics(batch_data_file=batch_data_file,
                                             iteration_data_file=data)
    ...
    if plot_tests:
        ...
        plot_3D_projected_points(pose_data=pose_data, 
    ...
    # Set paths
    DATA_DIR = "./data"
    obj_filename = "fitter_3d/ATTA_BOI/SMIL_model.obj"
    ...
    # CURRENTLY SHAPE BETAS ARE DISABLED / set to zero
    # model parameters
    ...
    cameras = FoVPerspectiveCameras(device=device, R=R, T=T, fov=cam_fov, degrees=True, znear=0.01, zfar=5000)
    ...
    keypoints_3d = np.array([[pose_data[key]["3DPos"]["x"],
                          pose_data[key]["3DPos"]["y"],
                          pose_data[key]["3DPos"]["z"]] for key in pose_data.keys()])

    # visualize keypoints with mirroring enabled
    sphere_meshes = create_sphere_meshes_at_points(keypoints_3d, radius=0.5, device=device, use_rainbow=True, mirror_x=True)
