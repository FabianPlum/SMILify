...
class SLEAPDataset(torch.utils.data.Dataset):
    ...
    def _load_metadata(self):
        ...
        with h5py.File(self.hdf5_path, "r") as f:
            ...
            self.is_sleap_dataset = metadata.attrs.get(
                "is_sleap_dataset", True
            )  # Flag for SLEAP-specific loss computation
            ...
            for group in [images_group, parameters_group, keypoints_group]:
                for dataset_name in group.keys():
                    ...
                    if len(dataset) != num_samples_expected:
                        raise ValueError(
                            f"Dataset {group.name}/{dataset_name} has {len(dataset)} samples, expected {num_samples_expected}"
                        )
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        with h5py.File(self.hdf5_path, "r") as f:
            ...
            crop_offset_yx = (
                f["auxiliary/crop_offset_yx"][idx]
                if "crop_offset_yx" in f["auxiliary"]
                else np.array([0, 0], dtype=np.int32)
            )
            crop_size_hw = (
                f["auxiliary/crop_size_hw"][idx]
                if "crop_size_hw" in f["auxiliary"]
                else np.array([self.target_resolution, self.target_resolution], dtype=np.int32)
            )
            scale_yx = (
                f["auxiliary/scale_yx"][idx] if "scale_yx" in f["auxiliary"] else np.array([1.0, 1.0], dtype=np.float32)
            )
