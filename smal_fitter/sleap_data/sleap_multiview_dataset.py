...
class SLEAPMultiViewDataset(torch.utils.data.Dataset):
        ...
        with h5py.File(self.hdf5_path, "r") as f:
            ...
            if "world_scale" not in metadata.attrs and (self.has_camera_parameters or self.has_3d_keypoints):
                try:
                    inferred = None
                    if (
                        self.has_camera_parameters
                        and "multiview_keypoints" in f
                        and "camera_extrinsics_t" in f["multiview_keypoints"]
                    ):
                    ...
                    if (
                        inferred is None
                        and self.has_3d_keypoints
                        and "multiview_keypoints" in f
                        and "keypoints_3d" in f["multiview_keypoints"]
                    ):
            ...
            self.has_pose_flag = "auxiliary" in f and "has_ground_truth_pose" in f["auxiliary"]

    @staticmethod
    def _sleap_to_pytorch3d_camera(
        R_cv: np.ndarray, t_cv: np.ndarray, K: np.ndarray, image_size_wh: np.ndarray
    ...
    def _validate_dataset(self):
        ...
        with h5py.File(self.hdf5_path, "r") as f:
            ...
            required_groups = ["multiview_images", "multiview_keypoints", "parameters", "auxiliary", "metadata"]
    ...
    def _pose_available(self, idx: int, global_rot: np.ndarray, joint_rot: np.ndarray, trans: np.ndarray) -> bool:
        ...
        return bool(np.any(global_rot != 0.0) or np.any(joint_rot != 0.0) or np.any(trans != 0.0))
    ...
    def _get_multiview_sample(self, idx: int) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        if self.num_views_to_use is not None and num_available > self.num_views_to_use:
            ...
            if self.random_view_sampling:
                selected_indices = np.random.choice(available_view_indices, size=self.num_views_to_use, replace=False)
        ...
        camera_names = [all_camera_names[i] if i < len(all_camera_names) else f"cam_{i}" for i in range(num_available)]
        camera_names = [
            camera_names[np.where(available_view_indices == v)[0][0]]
            for v in selected_indices
            if v in available_view_indices
        ]
        ...
        if self.augment:
            ...
            for vi in range(len(images)):
                ...
                if shared_scale is not None:
                    images[vi], K_aug[vi], kp2d[vi], vis[vi] = self._apply_geometric_augmentation(
                        images[vi],
                        K_aug[vi],
                        kp2d[vi],
                        vis[vi],
                ...
                images[vi] = self._apply_photometric_augmentation(
                    images[vi],
                    shared_params=shared_photometric,
                )
    ...
    def _get_single_view_sample(
        ...
        try:
            ...
            if self.has_camera_parameters and "camera_intrinsics" in f["multiview_keypoints"]:
                ...
                t = f["multiview_keypoints/camera_extrinsics_t"][idx, selected_view].astype(np.float32) * float(
                    self.world_scale
                )
    ...
    def _sample_photometric_params(self) -> Dict[str, Any]:
        ...
        if self.aug_color_jitter_saturation > 0:
            params["saturation_factor"] = float(
                np.random.uniform(
    ...
    def _apply_photometric_augmentation(
        ...
        if self.aug_gaussian_noise_std > 0:
            noise = np.random.normal(0, self.aug_gaussian_noise_std, image.shape).astype(np.float32)
            image = image + noise
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_views", type=int, default=None, help="Number of views to use per sample")
    parser.add_argument("--single_view", action="store_true", help="Test single-view mode")
    args = parser.parse_args()

    # Load dataset
    dataset = SLEAPMultiViewDataset(
        args.hdf5_path, num_views_to_use=args.num_views, return_single_view=args.single_view
    )
