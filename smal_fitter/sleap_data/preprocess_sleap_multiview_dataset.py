...
class SLEAPMultiViewPreprocessor:
    ...
    def _discover_multiview_frames(
        self, loader: SLEAPDataLoader, session_path: str, reproj_file: Optional[h5py.File] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if self.use_reprojections and reproj_file is not None:
            ...
            if cameras_to_remove:
                print(
                    f"  Excluded {len(cameras_to_remove)} camera(s) not in reprojections.h5 "
        ...
        for frame_idx in sorted(all_frames):
            available_cameras = [cam for cam, frames in camera_frames.items() if frame_idx in frames]
    ...
    def _get_annotated_frames(self, camera_data: Dict[str, Any], data_structure_type: str) -> List[int]:
        ...
        try:
            ...
            if self.load_3d_data:
                try:
                    ...
                    print(
                        f"Loaded 3D data for {Path(session_path).name}: {n_frames_3d} frames, {loader_3d.n_keypoints} keypoints"
                    )
                except Exception as e:
            ...
            if self.use_reprojections:
                ...
                reproj_candidates = sorted(session_path_obj.glob("reprojections*.h5")) or sorted(
                    session_path_obj.glob("*/reprojections*.h5")
                )
            ...
            print(
                f"Found {len(multiview_frames)} multi-view frames in {Path(session_path).name} ({num_cameras} cameras)"
            )
            ...
            for frame_info in tqdm(multiview_frames, desc=f"Processing {Path(session_path).name}", leave=False):
                try:
    ...
    def _process_multiview_frame(
        ...
        reproj_handle: Optional[h5py.File] = None,
        ...
        total_cameras_for_frame = (
            len(self.canonical_camera_order) if self.canonical_camera_order else len(available_cameras)
        )
                ...
                if frame_idx < loader_3d.n_frames:
                    ...
                    if keypoints_3d is not None:
                        ...
                        keypoints_3d, outlier_joint_mask = self._filter_outlier_3d_keypoints(
                            keypoints_3d, joint_names=joint_names
                        )
        ...
        for camera_name in available_cameras:
            ...
            try:
                ...
                if actual_pos != frame_idx:
                    print(
                        f"Warning: Position mismatch for {camera_name}! Expected frame {frame_idx}, got {actual_pos}. Seeking to correct position."
                    )
                ...
                if self.use_reprojections and reproj_handle is not None:
                    ...
                    if ds is not None:
                        try:
                            ...
                            if self.debug:
                                print(
                                    f"    Warning: Failed to read reprojection for {camera_name} frame {frame_idx}: {e}"
                                )
                    else:
                ...
                if has_3d_data and loader_3d is not None:
                    try:
                        ...
                        view_image_sizes.append(
                            np.array([self.target_resolution, self.target_resolution], dtype=np.int32)
                        )
                    except Exception as e:
        ...
        sample = {
            ...
            "camera_intrinsics": np.stack(view_camera_intrinsics, axis=0)
            if view_camera_intrinsics
            else None,  # (num_views, 3, 3)
            "camera_extrinsics_R": np.stack(view_camera_extrinsics_R, axis=0)
            if view_camera_extrinsics_R
            else None,  # (num_views, 3, 3)
            "camera_extrinsics_t": np.stack(view_camera_extrinsics_t, axis=0)
            if view_camera_extrinsics_t
            else None,  # (num_views, 3)
            "image_sizes": np.stack(view_image_sizes, axis=0)
            if view_image_sizes
            else None,  # (num_views, 2) - (width, height)
            ...
            "betas": ground_truth_betas.astype(np.float32)
            if ground_truth_betas is not None
            else np.zeros(config.N_BETAS, dtype=np.float32),
    ...
    def _filter_outlier_3d_keypoints(
        self, keypoints_3d: np.ndarray, joint_names: Optional[List[str]] = None, outlier_threshold_std: float = 4.0
        ...
        valid_mask = ~(
            np.isnan(keypoints_3d).any(axis=1) | np.isinf(keypoints_3d).any(axis=1) | (keypoints_3d == 0).all(axis=1)
        )
        ...
        if outlier_mask.any() and joint_names is not None and self.debug:
            ...
            outlier_names = [joint_names[i] if i < len(joint_names) else f"joint_{i}" for i in outlier_indices]
    ...
    def _undistort_image_and_keypoints(
        self, image: np.ndarray, keypoints_2d: np.ndarray, K: np.ndarray, distortion_coeffs: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _resolve_reprojection_dataset(
        self, reproj_handle: h5py.File, camera_name: str, warn: bool = True
        ...
        if warn:
            print(
                f"Warning: Could not resolve reprojection dataset for camera '{camera_name}' "
    ...
    def _preprocess_image(
        self, image: np.ndarray, keypoints_2d: Optional[np.ndarray] = None
    ...
    def _adjust_keypoints_for_transform(self, keypoints_2d: np.ndarray, transform_info: Dict[str, Any]) -> np.ndarray:
    ...
    def _adjust_intrinsics_for_transform(self, K: np.ndarray, transform_info: Dict[str, Any]) -> np.ndarray:
        """
    ...
    def process_dataset(self, sessions_dir: str, output_path: str, verbose: bool = True) -> Dict[str, Any]:
        """
    ...
    def _save_to_hdf5(self, samples: List[Dict[str, Any]], output_path: str):
        ...
        with h5py.File(output_path, "w") as f:
            ...
            images_group.create_dataset(
                "view_mask", data=view_mask_data, compression=self.compression, compression_opts=self.compression_level
            )

            keypoints_group.create_dataset(
                "keypoints_2d",
                data=keypoints_2d_data,
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            keypoints_group.create_dataset(
                "keypoint_visibility",
                data=keypoint_visibility_data,
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            keypoints_group.create_dataset(
                "camera_indices",
                data=camera_indices_data,
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            ...
            keypoints_group.create_dataset(
                "camera_intrinsics",
                data=camera_intrinsics_data,
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            keypoints_group.create_dataset(
                "camera_extrinsics_R",
                data=camera_extrinsics_R_data,
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            keypoints_group.create_dataset(
                "camera_extrinsics_t",
                data=camera_extrinsics_t_data,
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            keypoints_group.create_dataset(
            ...
            keypoints_group.create_dataset(
                "keypoints_3d",
                data=keypoints_3d_data,
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            auxiliary_group.create_dataset(
                "has_3d_data",
                data=np.array(has_3d_data_list),
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            ...
            parameters_group.create_dataset(
                "global_rot",
                data=np.array(global_rot_data, dtype=np.float32),
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            parameters_group.create_dataset(
                "joint_rot",
                data=np.array(joint_rot_data, dtype=np.float32),
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            parameters_group.create_dataset(
                "betas",
                data=np.array(betas_data, dtype=np.float32),
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            parameters_group.create_dataset(
                "trans",
                data=np.array(trans_data, dtype=np.float32),
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            ...
            auxiliary_group.create_dataset(
                "num_views",
                data=np.array(num_views_data),
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            auxiliary_group.create_dataset(
                "has_ground_truth_betas",
                data=np.array(has_gt_betas_data),
                compression=self.compression,
                compression_opts=self.compression_level,
            )
            auxiliary_group.create_dataset(
            ...
            metadata_group.attrs["views_excluded_position_correction_failed"] = self.stats[
                "views_excluded_position_correction_failed"
            ]
...
Examples:
  ...
  python preprocess_sleap_multiview_dataset.py /path/to/sleap/sessions multiview_sleap.h5 \\
    ...
    parser.add_argument(
        "--joint_lookup_table", type=str, default=None, help="Path to CSV lookup table for joint name mapping"
    )
    parser.add_argument(
        "--shape_betas_table", type=str, default=None, help="Path to CSV lookup table for ground truth shape betas"
    )
    ...
    parser.add_argument(
        "--target_resolution", type=int, default=None, help="(Deprecated, use --resolution) Target image resolution"
    )
    parser.add_argument(
        "--backbone",
        dest="backbone_name",
    ...
    parser.add_argument("--jpeg_quality", type=int, default=95, help="JPEG compression quality 1-100 (default: 95)")
    parser.add_argument(
        "--crop_mode",
        type=str,
        default="centred",
    ...
    parser.add_argument("--min_views", type=int, default=2, help="Minimum views required per sample (default: 2)")
    parser.add_argument(
        "--max_frames_per_session", type=int, default=None, help="Maximum frames to process per session (default: all)"
    )
    parser.add_argument(
        "--frame_skip",
        type=int,
    ...
    # Other options
    parser.add_argument(
        "--confidence_threshold", type=float, default=0.5, help="Keypoint confidence threshold (default: 0.5)"
    )
    ...
    try:
        ...
        stats = preprocessor.process_dataset(
            sessions_dir=args.sessions_dir, output_path=args.output_path, verbose=not args.quiet
        )
        ...
        if not args.quiet:
            ...
            if stats.get("samples_with_outliers_filtered", 0) > 0:
                print(
                    f"  Outlier 3D keypoints filtered: {stats['total_outlier_keypoints_filtered']} "
