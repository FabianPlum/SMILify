...
def test_preprocessing(
    ...
    try:
        stats = preprocessor.process_dataset(
            sessions_dir=sessions_dir, output_path=output_path, num_workers=num_workers, verbose=True
        )
    ...
    try:
        ...
        if len(dataset) > 0:
            ...
            for i in range(min(3, len(dataset))):
                ...
                print(
                    f"  Sample {i}: {x_data['camera_name']}/{x_data['frame_idx']} - {y_data['visible_keypoints_count']} visible keypoints"
                )
...
def main():
    ...
    parser.add_argument(
        "--skip_preprocessing", action="store_true", help="Skip preprocessing and test existing HDF5 file"
    )
