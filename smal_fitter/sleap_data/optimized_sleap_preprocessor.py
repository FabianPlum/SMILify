...
class OptimizedSLEAPDatasetPreprocessor:
    ...
    def process_single_session_optimized(self, session_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for camera_name in loader.camera_views:
                try:
                    camera_samples = self._process_camera_optimized(loader, camera_name, session_path)
    ...
    def _process_camera_optimized(
        self, loader: SLEAPDataLoader, camera_name: str, session_path: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _process_frame_optimized(
        ...
        try:
            ...
            smal_keypoints, smal_visibility = loader.map_keypoints_to_smal_model(keypoints_2d, visibility, image_size)
    ...
    def process_dataset(
        ...
        if num_workers > 1:
            ...
            with ProcessPoolExecutor(max_workers=num_workers) as executor:
                # Submit all sessions
                future_to_session = {
                    executor.submit(self.process_single_session_optimized, session): session for session in sessions
                }
        ...
        if verbose:
            ...
            print(
                f"  Average samples per video: {self.stats['processed_samples'] / max(self.stats['video_files_opened'], 1):.1f}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Optimized SLEAP dataset preprocessing", formatter_class=argparse.RawDescriptionHelpFormatter
    )
    ...
    parser.add_argument("--target_resolution", type=int, default=224, help="Target image resolution in pixels")
    parser.add_argument(
        "--backbone",
        dest="backbone_name",
    ...
    parser.add_argument("--jpeg_quality", type=int, default=95, help="JPEG compression quality 1-100")
    parser.add_argument("--chunk_size", type=int, default=8, help="HDF5 chunk size")
    parser.add_argument("--num_workers", type=int, default=4, help="Number of parallel processing workers")
    ...
    if not args.quiet:
        ...
        print(
            f"  I/O operations reduced by ~{((stats['total_samples'] - stats['video_files_opened']) / stats['total_samples'] * 100):.1f}%"
        )
