...
class SLEAPDatasetPreprocessor:
    ...
    def _process_camera_optimized(
        self, loader: SLEAPDataLoader, camera_name: str, session_path: str, reproj_file: Optional[h5py.File] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            frame_pbar = tqdm(annotated_frames, desc=f"Processing {camera_name}", leave=False, unit="frame")
    ...
    def _extract_2d_keypoints_for_frame(
        self,
        loader: SLEAPDataLoader,
        camera_data: Dict[str, Any],
        camera_name: str,
        frame_idx: int,
        reproj_handle: Optional[h5py.File],
            ...
            for sub in base.iterdir():
                ...
                if annotation_files:
                    ...
                    if candidates:
                        ...
                        if not self._reproj_warned_missing:
                            print(
                                f"Info: Annotation directory found ({sub}) but no reprojections*.h5 for camera {camera_name}"
                            )
    ...
    def _process_single_sample(
        self, loader: SLEAPDataLoader, camera_name: str, frame_idx: int, session_path: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            sample_data = {
                ...
                "joint_rot": np.zeros(
                    (config.N_POSE + 1, 3), dtype=np.float32
                ),  # Include root joint for consistency  # Placeholder
                "betas": ground_truth_betas
                if ground_truth_betas is not None
                else np.zeros(config.N_BETAS, dtype=np.float32),
    ...
    def process_dataset(
        self, sessions_dir: str, output_path: str, num_workers: int = 4, verbose: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if num_workers > 1:
            ...
            with ProcessPoolExecutor(max_workers=num_workers) as executor:
                # Submit all sessions
                future_to_session = {
                    executor.submit(self.process_single_session, session): session for session in sessions
                }

                # Collect results with progress bar
                if verbose:
                    futures = tqdm(as_completed(future_to_session), total=len(sessions), desc="Processing sessions")
                else:
    ...
    def _save_to_hdf5(self, samples: List[Dict[str, Any]], output_path: str):
        ...
        with h5py.File(output_path, "w") as f:
            ...
            for sample in samples:
                ...
                crop_size_hw_data.append(
                    sample.get(
                        "crop_size_hw", np.array([self.target_resolution, self.target_resolution], dtype=np.int32)
                    )
                )
...
Examples:
  ...
  python preprocess_sleap_dataset.py /path/to/sleap/sessions optimized_sleap.h5 \\
    ...
    parser.add_argument("output_path", help="Output HDF5 file path (e.g., optimized_sleap_dataset.h5)")
    ...
    parser.add_argument(
        "--target_resolution", type=int, default=224, help="Target image resolution in pixels (default: 224)"
    )
    parser.add_argument(
        "--backbone",
        dest="backbone_name",
    ...
    # Performance options
    parser.add_argument(
        "--chunk_size", type=int, default=8, help="HDF5 chunk size - should match training batch size (default: 8)"
    )
    parser.add_argument("--num_workers", type=int, default=1, help="Number of parallel processing workers (default: 4)")
    parser.add_argument(
    ...
    parser.add_argument("--validate", action="store_true", help="Validate the output dataset after preprocessing")
    parser.add_argument(
        "--use_reprojections",
        action="store_true",
