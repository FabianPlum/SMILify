...
def compute_axis_limits(keypoints_3d: np.ndarray, padding: float = 0.1):
    ...
    valid_mask = ~(
        np.isnan(keypoints_3d).any(axis=2) | np.isinf(keypoints_3d).any(axis=2) | (keypoints_3d == 0).all(axis=2)
    )
...
def create_3d_animation(
    keypoints_3d: np.ndarray, fps: int = 10, sample_skip: int = 1, padding: float = 0.1, point_size: float = 20.0
):
    """
    ...
    def update(frame):
        ...
        valid_mask = ~(np.isnan(kp3d).any(axis=1) | np.isinf(kp3d).any(axis=1) | (kp3d == 0).all(axis=1))
        ...
        ax.set_zlabel("Z")
        ax.set_title(
            f"3D Keypoints - Sample {sample_idx + 1}/{num_samples} ({valid_mask.sum()}/{n_joints} valid joints)"
        )
    ...
    anim = animation.FuncAnimation(fig, update, interval=interval_ms, blit=False, repeat=True)
...
Examples:
  ...
  python visualize_3d_keypoints.py multiview_sleap.h5 --point_size 50
    ...
    parser.add_argument("--sample_skip", type=int, default=1, help="Process every Nth sample (default: 1, all samples)")
    parser.add_argument("--padding", type=float, default=0.1, help="Axis padding factor (default: 0.1 = 10%%)")
    parser.add_argument("--point_size", type=float, default=20.0, help="Size of scatter plot points (default: 20.0)")
    ...
    try:
        create_3d_animation(
            keypoints_3d, fps=args.fps, sample_skip=args.sample_skip, padding=args.padding, point_size=args.point_size
        )
