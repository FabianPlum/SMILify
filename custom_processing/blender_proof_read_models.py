...
def register():
    ...
    if not hasattr(bpy.types.Scene, "input_folder"):
        bpy.types.Scene.input_folder = StringProperty(
            name="Input Folder", description="Folder containing .obj files", subtype="DIR_PATH"
        )
    if not hasattr(bpy.types.Scene, "output_folder"):
        bpy.types.Scene.output_folder = StringProperty(
            name="Output Folder", description="Folder to save processed .obj files", subtype="DIR_PATH"
        )
    ...
    for cls in [
        MODEL_LOADER_PT_Panel,
        MODEL_LOADER_OT_LoadNextModel,
        MODEL_LOADER_OT_LoadPreviousModel,
        MODEL_LOADER_OT_ExportModel,
    ]:
