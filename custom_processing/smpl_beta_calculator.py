...
class SMPLBetaCalculator:
    ...
    def _beta_from_height_weight(
        self, height: float, weight: float, a: float, b: float, A: np.ndarray, B: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def calculate_betas(self, measurements: List[float], gender: Union[Gender, str]) -> np.ndarray:
        """
        ...
        if num_measurements not in [2, 4, 5, 6]:
            raise ValueError(
                f"Invalid number of measurements: {num_measurements}. Expected 2, 4, 5, or 6 measurements."
            )
        ...
        if num_measurements == 2:
            ...
            return self._beta_from_height_weight_armspan_inseam(height, weight, armspan, inseam, a, b, A4, B4)

        elif num_measurements == 5:
            armspan = measurements[2]
            inseam = measurements[3]
            inseam_width = measurements[4]
            return self._beta_from_regressor5(height, weight, armspan, inseam, inseam_width, a, b, A5, B5)
            ...
            return self._beta_from_regressor6(
                height, weight, armspan, inseam, inseam_width, wrist_to_shoulder, a, b, A6, B6
            )
...
def demo():
    ...
    print(
        f"Input: height={measurements_4[0]}m, weight={measurements_4[1]}kg, "
    ...
    print(
        f"Input: height={measurements_6[0]}m, weight={measurements_6[1]}kg, "
