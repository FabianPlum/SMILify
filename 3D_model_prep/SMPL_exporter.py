...
def export_y_axis_vertices_to_npy(obj, filepath):
    ...
    y_axis_vertices = np.array(
        [i for i, vert in enumerate(mesh.vertices) if np.isclose(vert.co.y, 0.0, atol=1e-3)], dtype=int
    )
...
def test_export_functions():
    ...
    try:
        j_regressor_path = export_J_regressor_to_npy(obj, armature_obj, 5, j_regressor_npy_path)[
            0
        ]  # Assuming 5 closest vertices
