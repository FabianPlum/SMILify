...
def process_mesh_folder(
    input_dir: str, output_dir: str, num_samples: int = -1, num_rays: int = 30, k_smoothing: int = 50
) -> None:
    """
    ...
    for mesh_file in tqdm(mesh_files, desc="Processing meshes"):
        ...
        try:
            ...
            visualize_sdf(
                mesh, sample_points, smoothed_diameters, plot_path, title=f"Spatial Diameter Function - {mesh_name}"
            )
            ...
            visualize_vertex_sdf(mesh, vertex_sdf, vertex_plot_path, title=f"Vertex SDF - {mesh_name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute and visualize Spatial Diameter Function for multiple meshes")
    parser.add_argument(
        "input_dir", type=str, help="Directory containing input OBJ files (will be searched recursively)"
    )
    parser.add_argument(
        "--output_dir", type=str, default="sdf_batch_output", help="Directory to save the visualizations and data"
    )
    parser.add_argument(
        "--num_samples", type=int, default=-1, help="Number of faces to sample. If -1, samples all faces (default: -1)"
    )
    parser.add_argument("--num_rays", type=int, default=30, help="Number of rays to cast per sampled point")
    parser.add_argument("--k_smoothing", type=int, default=50, help="Number of neighbors for smoothing")
