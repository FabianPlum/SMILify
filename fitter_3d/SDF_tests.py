...
class PerformanceMonitor:
    ...
    def get_cpu_stats(self):
        return {"cpu_util": psutil.cpu_percent(interval=None), "memory_util": psutil.virtual_memory().percent}
...
def compute_ray_mesh_intersections_vectorized(
    origin, direction, verts, faces, exclude_face_idx=None, exclude_radius=0.01
):
    """
...
def compute_sdf(mesh: Meshes, num_samples: int = 1000, num_rays: int = 30):
    ...
    if num_samples == len(faces):
        ...
        sample_points = (
            w0.unsqueeze(-1) * face_verts[:, 0]
            + w1.unsqueeze(-1) * face_verts[:, 1]
            + w2.unsqueeze(-1) * face_verts[:, 2]
        )  # [num_samples, 3]
    ...
    for batch in tqdm(range(num_batches), desc="Processing batches"):
        ...
        for i in range(batch_size_actual):
            ...
            for direction in point_directions:
                if found_valid and len(valid_distances) >= max(
                    int(num_samples / 2), 1
                ):  # Early termination if we have enough valid samples
                    break
...
def visualize_sdf(
    ...
    output_path: str,
    title: str = "Spatial Diameter Function",
):
    """
    ...
    scatter = ax.scatter(points_np[:, 0], points_np[:, 1], points_np[:, 2], c=diameters_np, cmap="RdYlGn_r", s=50)
...
def debug_single_vertex(mesh: Meshes, num_rays: int = 30, output_dir: str = "sdf_output", seed: int = 0):
    ...
    sample_point = w0 * face_verts[0] + w1 * face_verts[1] + w2 * face_verts[2]  # [3]

    # Compute face normal
    normal = torch.cross(face_verts[1] - face_verts[0], face_verts[2] - face_verts[0])
    normal = normal / torch.norm(normal)
    ...
    if all_intersections:
        ...
        visualize_rays(ax, offset_origin, all_directions, all_distances, all_intersections, color="blue", alpha=0.2)
    else:
    ...
    if valid_intersections:
        ...
        ax.scatter(
            valid_intersections[:, 0].cpu(),
            valid_intersections[:, 1].cpu(),
            valid_intersections[:, 2].cpu(),
            color="green",
            s=50,
            label="Valid Intersections",
        )
    ...
    ax.plot(
        [sample_point[0].cpu(), normal_end[0].cpu()],
...
def assign_vertex_sdf(
    verts: torch.Tensor, sample_points: torch.Tensor, smoothed_diameters: torch.Tensor, k: int = 10
) -> torch.Tensor:
    """
...
def visualize_vertex_sdf(mesh: Meshes, vertex_sdf: torch.Tensor, output_path: str, title: str = "Vertex SDF"):
    ...
    # Create scatter plot with custom colormap
    scatter = ax.scatter(verts_np[:, 0], verts_np[:, 1], verts_np[:, 2], c=sdf_np, cmap="RdYlGn_r", s=50)
...
def process_obj_file(
    ...
    visualize_sdf(mesh, sample_points, smoothed_diameters, output_path, title=f"Spatial Diameter Function - {basename}")
    ...
    visualize_vertex_sdf(mesh, vertex_sdf, vertex_output_path, title=f"Vertex SDF - {basename}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="sdf_batch_output", help="Directory to save the visualization"
    )
    parser.add_argument(
        "--num_samples",
        type=int,
    ...
    parser.add_argument("--num_rays", type=int, default=30, help="Number of rays to cast per sampled point")
    parser.add_argument("--debug", action="store_true", help="Run in debug mode with single vertex visualization")
    parser.add_argument("--seed", type=int, default=123, help="Random seed for reproducible testing")
